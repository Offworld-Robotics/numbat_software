...
armJointVel ArmControl::convertJoystickMessageToJoints(const sensor_msgs::Joy::ConstPtr& joy) {
    armJointVel output;
    // the arm upper actuator is a number from -1 to 1 that corresponds
    // to the y-position of the left joystick
    output.armUpperActuator = joy->axes[ARM_STICK_UPPER_UD];
    // the arm lower actuator is a number from -1 to 1 that corresponds
    // to the y-position of the right joystick
