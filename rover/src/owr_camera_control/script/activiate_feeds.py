...
def callback(data):
    ...
    if data.on:
        print "on"
        os.environ["GSCAM_CONFIG"] = "v4l2src device=/dev/video"+str(data.stream)+" ! video/x-raw-rgb,framerate=25/1,width=640,height=480 ! ffmpegcolorspace"
        #subprocess.Popen([ "rosrun","gscam", "gscam","__name=" + str(data.stream) + "_camera","gscam_publisher:=\/cam" + str(data.stream)])
        #subprocess.Popen([ "rosrun","gscam", "gscam","__name:=" + "cam" + str(data.stream),"/camera/image_raw:=/cam" + str(data.stream)])
        subprocess.Popen([ "rosrun","gscam", "gscam"])
    else:
